use crate::{
    ...
    solana_client::{
        build_compute_budget_instructions, build_instruction_deposit,
        build_instruction_initialize_vault, build_instruction_withdraw, load_deployer_keypair,
        send_transaction_with_retries, DepositParams, WithdrawParams,
    },
};
use solana_sdk::{
...
impl VaultManager {
    ...
    pub async fn submit_withdraw(&self, owner: &Pubkey, amount: u64) -> AppResult<String> {
        ...
        let mint = Pubkey::from_str(&self.state.cfg.usdt_mint).unwrap_or_default();
        let wd_ix = build_instruction_withdraw(&WithdrawParams {
            program_id,
            owner: *owner,
            mint,
            amount,
    ...
    pub async fn query_balance_by_owner(&self, owner: &str) -> AppResult<u64> {
        ...
        Err(AppError::BadRequest(
            "invalid owner or token account".to_string(),
        ))
    }

    pub async fn available_balance(&self, owner: &str) -> AppResult<i64> {
        let (_token_opt, total) = db::get_vault(&self.state.pool, owner)
            .await?
            .unwrap_or((None, 0));
        let locked = db::get_locked_balance(&self.state.pool, owner)
            .await
            .unwrap_or(0);
        Ok(total - locked)
    }
