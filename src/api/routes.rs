...
use crate::{
    ...
    db,
    solana_client::{
        build_compute_budget_instructions, build_instruction_add_withdraw_whitelist,
        build_instruction_add_yield_program, build_instruction_compound_yield,
        build_instruction_deposit, build_instruction_emergency_withdraw,
        build_instruction_initialize_vault, build_instruction_remove_withdraw_whitelist,
        build_instruction_remove_yield_program, build_instruction_request_withdraw,
        build_instruction_schedule_timelock, build_instruction_set_risk_level,
        ...
        build_instruction_yield_deposit, build_instruction_yield_withdraw,
        build_partial_withdraw_tx, fetch_vault_multisig_config, load_deployer_keypair,
        send_transaction_with_retries, AddYieldProgramParams, CompoundYieldParams, DepositParams,
        EmergencyWithdrawParams, RemoveYieldProgramParams, SetRiskLevelParams,
        TransferCollateralParams, WithdrawMultisigParams, WithdrawParams, YieldDepositParams,
        YieldWithdrawParams,
    },
};
...
pub async fn ready(State(state): State<AppState>) -> impl IntoResponse {
    let db_ok = sqlx::query_scalar::<_, i64>("SELECT 1 as one")
    ...
    (
        StatusCode::OK,
        Json(serde_json::json!({ "status": status, "db": db_ok, "rpc": rpc_ok })),
    )
}
...
pub async fn vault_initialize(
    ...
    let program_id =
        Pubkey::from_str(&state.cfg.program_id).unwrap_or(solana_sdk::pubkey::Pubkey::default());
    let user = match Pubkey::from_str(&req.user_pubkey) {
        Ok(p) => p,
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({ "error": "invalid user_pubkey" })),
            )
        }
    };
    let usdt_mint = match Pubkey::from_str(&state.cfg.usdt_mint) {
        ...
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({ "error": "invalid usdt mint" })),
            )
        }
    };
...
pub async fn vault_deposit(
    ...
    let program_id = match Pubkey::from_str(&state.cfg.program_id) {
        Ok(p) => p,
    ...
    match db::consume_nonce(&state.pool, &req.nonce, &req.owner).await {
        ...
        Ok(false) => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({ "error": "invalid or used nonce" })),
            )
        }
        Err(e) => {
            return (
    ...
    let mint = match Pubkey::from_str(&state.cfg.usdt_mint) {
        Ok(p) => p,
        Err(_) => Pubkey::default(),
    };
    let ix = match build_instruction_deposit(&DepositParams {
        program_id,
        owner,
        mint,
        amount: req.amount,
    ...
    let _ = db::insert_audit_log(
        ...
        Some(&req.owner),
        "deposit_request",
        serde_json::json!({ "amount": req.amount, "nonce": req.nonce }),
    )
    .await;
    (
        StatusCode::OK,
        Json(serde_json::json!({ "instruction": payload })),
    )
}
...
pub async fn vault_withdraw(
    ...
    if let Ok(Some((secret, enabled))) = db::twofa_get(&state.pool, &req.owner).await {
        if enabled {
            ...
            if let Some(val) = headers.get("x-2fa-code").and_then(|v| v.to_str().ok()) {
                ...
                return (
                    StatusCode::UNAUTHORIZED,
                    Json(serde_json::json!({ "error": "2fa code required" })),
                );
            }
        }
    }
    ...
    let withdraw_ix = match build_instruction_withdraw(&WithdrawParams {
        program_id,
        owner,
        mint,
        amount: req.amount,
    ...
    let recent_blockhash = match state.sol.rpc.get_latest_blockhash().await {
        ...
        Err(e) => {
            return (
                ...
                Json(serde_json::json!({ "error": format!("blockhash: {e}") })),
            )
        }
    };
    ...
    if let Ok(owner_pk) = Pubkey::from_str(&req.owner) {
        ...
        if let Ok(chain_bal) = crate::solana_client::get_token_balance(&state.sol, &owner_pk).await
        {
            ...
            .await;
            let _ = state.notifier.vault_balance_tx.send(
                serde_json::json!({
                    "owner": req.owner,
                    "balance": chain_bal,
                })
    ...
    let _ = db::insert_audit_log(
        ...
        Some(&req.owner),
        "withdraw_submitted",
        serde_json::json!({ "amount": req.amount, "signature": sig.to_string() }),
    )
    .await;
    (
        StatusCode::OK,
...
pub async fn vault_balance(
    ...
    match Pubkey::from_str(&owner) {
        ...
        Err(_) => (
            StatusCode::BAD_REQUEST,
            Json(serde_json::json!({ "error": "invalid owner or token account" })),
        ),
    }
}
    ...
    match db::list_transactions(&state.pool, &owner, limit, offset).await {
        Ok(rows) => {
            let items: Vec<_> = rows
                .into_iter()
                .map(|(id, signature, amount, kind, created_at)| {
                    serde_json::json!({
                        "id": id,
                        "signature": signature,
                        "amount": amount,
...
pub async fn vault_schedule_withdraw(
    State(state): State<AppState>,
    Json(req): Json<ScheduleWithdrawRequest>,
) -> impl IntoResponse {
    // Verify signature on message: schedule:{owner}:{amount}:{duration}:{nonce}
    let message = format!(
        "schedule:{}:{}:{}:{}",
        req.owner, req.amount, req.duration_seconds, req.nonce
    );
    if let Err(e) = verify_wallet_signature(&req.owner, message.as_bytes(), &req.signature) {
        return (
            StatusCode::UNAUTHORIZED,
    ...
    let sched_ix =
        match build_instruction_schedule_timelock(&crate::solana_client::ScheduleTimelockParams {
            program_id,
            owner,
            amount: req.amount,
    ...
    let _ = db::insert_audit_log(
        ...
        Some(&req.owner),
        "timelock_scheduled",
        serde_json::json!({ "amount": req.amount, "unlock_at": unlock_at }),
    )
    .await;
    ...
    (
        StatusCode::OK,
        Json(serde_json::json!({ "signature": sig.to_string(), "unlock_at": unlock_at })),
    )
}

pub async fn vault_list_timelocks(
    State(state): State<AppState>,
    Path(owner): Path<String>,
) -> impl IntoResponse {
...
pub async fn vault_emergency_withdraw(
    ...
    if state.cfg.admin_jwt_secret.is_empty() {
        return (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(serde_json::json!({ "error": "admin secret not configured" })),
        );
    }
    ...
    let owner = match Pubkey::from_str(&req.owner) {
        ...
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({ "error": "invalid owner" })),
            )
        }
    };
    ...
    if !state.cfg.vault_authority_pubkey.is_empty() {
        if let Ok(cfg_va) = Pubkey::from_str(&state.cfg.vault_authority_pubkey) {
            if cfg_va != payer.pubkey() {
                return (
                    StatusCode::BAD_REQUEST,
                    Json(serde_json::json!({ "error": "payer is not governance signer" })),
                );
            }
        }
    }
    ...
    let em_ix = match build_instruction_emergency_withdraw(
    ...
    let _ = db::insert_audit_log(
        ...
        Some(&req.owner),
        "emergency_withdraw",
        serde_json::json!({ "amount": req.amount, "reason": req.reason }),
    )
    .await;
...
pub async fn vault_config(
    ...
    let program_id = match Pubkey::from_str(&state.cfg.program_id) {
        ...
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({ "error": "invalid program id" })),
            )
        }
    };
    ...
    let delegates = db::delegate_list(&state.pool, &owner)
        .await
        .unwrap_or_default();
    match ms {
        Ok((threshold, signers)) => {
            ...
            (
                StatusCode::OK,
                Json(
                    serde_json::json!({ "threshold": threshold, "signers": signers_str, "delegates": delegates }),
                ),
            )
        }
        Err(e) => (
            StatusCode::BAD_GATEWAY,
            Json(serde_json::json!({ "error": e.to_string(), "delegates": delegates })),
        ),
    }
}
...
// -----------------
#[derive(Deserialize)]
pub struct ProposeWithdrawRequest {
    pub owner: String,
    pub amount: u64,
    pub threshold: u8,
...
pub async fn vault_propose_withdraw(
    State(state): State<AppState>,
    Json(req): Json<ProposeWithdrawRequest>,
) -> impl IntoResponse {
    // Verify initiator signature on message: propose_withdraw:{owner}:{amount}:{threshold}:{nonce}
    let message = format!(
        "propose_withdraw:{}:{}:{}:{}",
        req.owner, req.amount, req.threshold, req.nonce
    );
    if let Err(e) = verify_wallet_signature(&req.owner, message.as_bytes(), &req.signature) {
        return (
            StatusCode::UNAUTHORIZED,
    ...
    let (threshold, signers) = if req.signers.is_empty() || req.threshold == 0 {
        ...
        let owner_pk = match Pubkey::from_str(&req.owner) {
            Ok(p) => p,
            Err(_) => Pubkey::default(),
        };
        ...
        for s in req.signers.iter() {
            uniq.insert(s);
        }
        if uniq.len() != req.signers.len() {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({ "error": "duplicate signers" })),
            );
        }
        if (req.threshold as usize) > req.signers.len() {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({ "error": "invalid threshold" })),
            );
        }
    ...
    if let Err(e) = db::ms_create_proposal(
        &state.pool,
        &id,
        &req.owner,
        req.amount as i64,
    ...
    (
        StatusCode::OK,
        Json(serde_json::json!({ "proposal_id": id })),
    )
}

#[derive(Deserialize)]
pub struct ApproveWithdrawRequest {
...
pub async fn vault_approve_withdraw(
    State(state): State<AppState>,
    Json(req): Json<ApproveWithdrawRequest>,
) -> impl IntoResponse {
    ...
    if status != "pending" {
        return (
            StatusCode::BAD_REQUEST,
            Json(serde_json::json!({ "error": "proposal not pending" })),
        );
    }
    // signer must be in allowed list
    let allowed: Vec<String> = match serde_json::from_value(signers_json.clone()) {
        Ok(v) => v,
        Err(_) => vec![],
    };
    if !allowed.iter().any(|s| s == &req.signer) {
        return (
            StatusCode::BAD_REQUEST,
            Json(serde_json::json!({ "error": "signer not allowed" })),
        );
    }
    ...
    if count as i32 >= threshold {
        ...
        let approvals = match db::ms_list_approvals(&state.pool, &req.proposal_id).await {
            Ok(v) => v,
            Err(_) => vec![],
        };
        let mut signer_pubkeys: Vec<Pubkey> = approvals
            .iter()
        ...
        let params = WithdrawMultisigParams {
            program_id,
            owner: owner_pk,
            authority: authority_pk,
            amount: amount as u64,
            other_signers: signer_pubkeys.clone(),
        };
        let payer = match load_deployer_keypair(&state.cfg.deployer_keypair_path) {
            Ok(kp) => kp,
            Err(e) => {
        ...
        let raw = match build_partial_withdraw_tx(&state.sol, &payer, &params).await {
            Ok(b) => b,
            Err(e) => {
                return (
                    StatusCode::BAD_GATEWAY,
        ...
        let signer_strings: Vec<String> = std::iter::once(req.signer.clone())
            .chain(approvals.into_iter().filter(|s| s != &req.signer))
            .collect();
        ...
        (
            StatusCode::OK,
            Json(
                serde_json::json!({ "ready": true, "transaction_base64": tx_b64, "required_signers": params.other_signers.iter().map(|p| p.to_string()).collect::<Vec<_>>() }),
            ),
        )
    } else {
        (
            StatusCode::OK,
            Json(serde_json::json!({ "ready": false, "approvals": count, "threshold": threshold })),
        )
    }
}

pub async fn vault_proposal_status(
    State(state): State<AppState>,
    Path(id): Path<String>,
) -> impl IntoResponse {
    ...
    let approvals = match db::ms_count_approvals(&state.pool, &id).await {
        Ok(c) => c,
        Err(_) => 0,
    };
    (
        StatusCode::OK,
        Json(
            serde_json::json!({ "status": status, "approvals": approvals, "threshold": threshold }),
        ),
    )
}
...
pub async fn vault_delegate_add(
    State(state): State<AppState>,
    Json(req): Json<DelegateAddRequest>,
) -> impl IntoResponse {
    if Pubkey::from_str(&req.owner).is_err() || Pubkey::from_str(&req.delegate).is_err() {
        return (
            StatusCode::BAD_REQUEST,
            Json(serde_json::json!({ "error": "invalid pubkey(s)" })),
        );
    }
    ...
    match db::delegate_add(&state.pool, &req.owner, &req.delegate).await {
        Ok(true) => {
            let _ = db::insert_audit_log(
                ...
                Some(&req.owner),
                "delegate_add",
                serde_json::json!({ "delegate": req.delegate }),
            )
            .await;
...
#[derive(Deserialize)]
pub struct DelegateRemoveRequest {
    pub owner: String,
    pub delegate: String,
    pub nonce: String,
    pub signature: String,
...
pub async fn vault_delegate_remove(
    State(state): State<AppState>,
    Json(req): Json<DelegateRemoveRequest>,
) -> impl IntoResponse {
    if Pubkey::from_str(&req.owner).is_err() || Pubkey::from_str(&req.delegate).is_err() {
        return (
            StatusCode::BAD_REQUEST,
    ...
    let message = format!(
        "delegate_remove:{}:{}:{}",
        req.owner, req.delegate, req.nonce
    );
    if let Err(e) = verify_wallet_signature(&req.owner, message.as_bytes(), &req.signature) {
        return (
            StatusCode::UNAUTHORIZED,
...
#[derive(Deserialize)]
pub struct AdminAddProgramRequest {
...
#[derive(Deserialize)]
pub struct AdminSetVaultTokenAccountRequest {
    pub owner: String,
...
pub async fn admin_set_vault_token_account(
    ...
    if let Err(e) =
        db::upsert_vault_token_account(&state.pool, &req.owner, &req.token_account).await
    {
        return (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(serde_json::json!({ "error": e.to_string() })),
    ...
    if let Ok(pk) = Pubkey::from_str(&req.token_account) {
        if let Ok(bal) = crate::solana_client::get_token_balance(&state.sol, &pk).await {
            ...
            let _ = state
                .notifier
                .vault_balance_tx
                .send(serde_json::json!({ "owner": req.owner, "balance": bal }).to_string());
        }
    }
    (StatusCode::OK, Json(serde_json::json!({ "ok": true })))
}
...
pub async fn pm_lock(
    State(state): State<AppState>,
    Json(req): Json<PmLockRequest>,
) -> impl IntoResponse {
    ...
    match mgr.lock(&owner, req.amount).await {
        Ok(sig) => {
            ...
            let _ = db::insert_audit_log(
                ...
                Some(&req.owner),
                "lock_submitted",
                serde_json::json!({ "amount": req.amount, "signature": sig }),
            )
            .await;
            let _ = state.notifier.lock_tx.send(
                serde_json::json!({"owner": req.owner, "amount": req.amount, "signature": sig})
                    .to_string(),
            );
...
pub async fn pm_unlock(
    State(state): State<AppState>,
    Json(req): Json<PmUnlockRequest>,
) -> impl IntoResponse {
    ...
    match mgr.unlock(&owner, req.amount).await {
        Ok(sig) => {
            ...
            .await;
            let _ = state.notifier.unlock_tx.send(
                serde_json::json!({"owner": req.owner, "amount": req.amount, "signature": sig})
                    .to_string(),
            );
            (StatusCode::OK, Json(serde_json::json!({"signature": sig})))
...
// -----------------
#[derive(Deserialize)]
pub struct YieldOpRequest {
    pub owner: String,
    pub amount: u64,
    pub yield_program: String,
    pub nonce: String,
    pub signature: String,
}

pub async fn vault_yield_deposit(
    State(state): State<AppState>,
    Json(req): Json<YieldOpRequest>,
) -> impl IntoResponse {
    let message = format!(
        "yield_deposit:{}:{}:{}:{}",
        req.owner, req.amount, req.yield_program, req.nonce
    );
    if let Err(e) = verify_wallet_signature(&req.owner, message.as_bytes(), &req.signature) {
        return (
            StatusCode::UNAUTHORIZED,
    ...
    let yield_program = match Pubkey::from_str(&req.yield_program) {
        Ok(p) => p,
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({ "error": "invalid yield_program" })),
            )
        }
    };
    let ix = match build_instruction_yield_deposit(&YieldDepositParams {
        program_id,
        owner,
        amount: req.amount,
        yield_program,
...
pub async fn vault_yield_withdraw(
    State(state): State<AppState>,
    Json(req): Json<YieldOpRequest>,
) -> impl IntoResponse {
    let message = format!(
    ...
    let ix = match build_instruction_yield_withdraw(&YieldWithdrawParams {
        program_id,
        owner,
        amount: req.amount,
        yield_program,
...
#[derive(Deserialize)]
pub struct YieldCompoundRequest {
    pub owner: String,
...
pub async fn vault_compound_yield(
    State(state): State<AppState>,
    Json(req): Json<YieldCompoundRequest>,
) -> impl IntoResponse {
    let message = format!(
        "compound_yield:{}:{}:{}:{}",
        req.owner, req.compounded_amount, req.yield_program, req.nonce
    );
    if let Err(e) = verify_wallet_signature(&req.owner, message.as_bytes(), &req.signature) {
        return (
            StatusCode::UNAUTHORIZED,
    ...
    let ix = match build_instruction_compound_yield(&CompoundYieldParams {
        program_id,
        owner,
        compounded_amount: req.compounded_amount,
        yield_program,
    }) {
        Ok(ix) => ix,
        Err(e) => {
...
pub async fn internal_transfer_collateral(
    ...
    let from_owner = match Pubkey::from_str(&req.from_owner) {
        Ok(p) => p,
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({ "error": "invalid from_owner" })),
            )
        }
    };
    let to_owner = match Pubkey::from_str(&req.to_owner) {
        Ok(p) => p,
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({ "error": "invalid to_owner" })),
            )
        }
    };
    let mint = match Pubkey::from_str(&state.cfg.usdt_mint) {
    ...
    let caller_program = if let Some(cp) = &req.caller_program {
        match Pubkey::from_str(cp) {
            Ok(p) => p,
            Err(_) => {
                return (
                    StatusCode::BAD_REQUEST,
                    Json(serde_json::json!({ "error": "invalid caller_program" })),
                )
            }
        }
    } else {
        match Pubkey::from_str(&state.cfg.position_manager_program_id) {
            Ok(p) => p,
            Err(_) => Pubkey::default(),
        }
    };
    ...
    let mut ixs = build_compute_budget_instructions(1_400_000, 1_000);
    let ix = match build_instruction_transfer_collateral(&TransferCollateralParams {
        program_id,
        caller_program,
        from_owner,
        to_owner,
...
pub async fn vault_yield_status(
    State(state): State<AppState>,
    Path(owner): Path<String>,
) -> impl IntoResponse {
    let apys = crate::db::latest_protocol_apys(&state.pool)
        .await
        .unwrap_or_default();
    ...
    let (selected, projected_apr) = match best {
        Some((p, a)) => (Some(p.to_string()), a),
        None => (None, 0.0),
    };
    ...
    if let (Ok(program_id), Ok(owner_pk)) = (
        Pubkey::from_str(&state.cfg.program_id),
        Pubkey::from_str(&owner),
    ) {
        if let Ok(snap) =
            crate::solana_client::fetch_vault_yield_info(&state.sol, &owner_pk, &program_id).await
        {
...
pub async fn admin_withdraw_whitelist_add(
    State(state): State<AppState>,
    Json(req): Json<AdminWhitelistReq>,
) -> impl IntoResponse {
    ...
    let ix = match build_instruction_add_withdraw_whitelist(&program_id, &owner, &address) {
        Ok(ix) => ix,
        Err(e) => {
            return (
                StatusCode::BAD_REQUEST,
    ...
    let _ = db::insert_audit_log(
        &state.pool,
        Some(&req.owner),
        "whitelist_add_requested",
        serde_json::json!({ "address": req.address }),
    )
    .await;
    (
        StatusCode::OK,
...
pub async fn admin_withdraw_whitelist_remove(
    State(state): State<AppState>,
    Json(req): Json<AdminWhitelistReq>,
) -> impl IntoResponse {
    if Pubkey::from_str(&req.owner).is_err() || Pubkey::from_str(&req.address).is_err() {
    ...
    let owner = Pubkey::from_str(&req.owner).unwrap();
    let address = Pubkey::from_str(&req.address).unwrap();
    let ix = match build_instruction_remove_withdraw_whitelist(&program_id, &owner, &address) {
        Ok(ix) => ix,
        Err(e) => {
            return (
                StatusCode::BAD_REQUEST,
...
#[derive(Deserialize)]
pub struct AdminMinDelayReq {
    pub owner: String,
    pub seconds: i64,
}

pub async fn admin_withdraw_min_delay_set(
    State(state): State<AppState>,
    Json(req): Json<AdminMinDelayReq>,
) -> impl IntoResponse {
    if Pubkey::from_str(&req.owner).is_err() {
        return (
            StatusCode::BAD_REQUEST,
            Json(serde_json::json!({ "error": "invalid owner" })),
        );
    ...
    let owner = Pubkey::from_str(&req.owner).unwrap();
    let ix = match build_instruction_set_withdraw_min_delay(&program_id, &owner, req.seconds) {
        Ok(ix) => ix,
        Err(e) => {
            return (
                StatusCode::BAD_REQUEST,
    ...
    let _ = db::insert_audit_log(
        &state.pool,
        Some(&req.owner),
        "min_delay_set_requested",
        serde_json::json!({ "seconds": req.seconds }),
    )
    .await;
    (
        StatusCode::OK,
...
#[derive(Deserialize)]
pub struct AdminRateLimitReq {
    pub owner: String,
    pub window_seconds: u32,
    pub max_amount: u64,
}

pub async fn admin_withdraw_rate_limit_set(
    State(state): State<AppState>,
    Json(req): Json<AdminRateLimitReq>,
) -> impl IntoResponse {
    if Pubkey::from_str(&req.owner).is_err() {
        return (
            StatusCode::BAD_REQUEST,
    ...
    let owner = Pubkey::from_str(&req.owner).unwrap();
    let ix = match build_instruction_set_withdraw_rate_limit(
        &program_id,
        &owner,
        req.window_seconds,
        req.max_amount,
    ...
    let _ = db::insert_audit_log(
        &state.pool,
        Some(&req.owner),
        "rate_limit_set_requested",
        serde_json::json!({ "window_seconds": req.window_seconds, "max_amount": req.max_amount }),
    )
    .await;
    (
        StatusCode::OK,
...
#[derive(Deserialize)]
pub struct AdminYieldProgramReq {
    pub yield_program: String,
}
...
pub async fn admin_yield_program_add(
    ...
    let yield_program = match Pubkey::from_str(&req.yield_program) {
        ...
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({ "error": "invalid yield program" })),
            )
        }
    };
    let payer = match load_deployer_keypair(&state.cfg.deployer_keypair_path) {
    ...
    let mut ixs = build_compute_budget_instructions(1_200_000, 1_000);
    let ix = match build_instruction_add_yield_program(&AddYieldProgramParams {
        program_id,
    ...
    let _ = db::insert_audit_log(
        ...
        None,
        "yield_program_add",
        serde_json::json!({ "yield_program": req.yield_program, "signature": signature }),
    )
    .await;
...
pub async fn admin_yield_program_remove(
    ...
    let mut ixs = build_compute_budget_instructions(1_200_000, 1_000);
    let ix = match build_instruction_remove_yield_program(&RemoveYieldProgramParams {
        program_id,
        governance: payer.pubkey(),
        yield_program,
    }) {
...
#[derive(Deserialize)]
pub struct AdminRiskLevelSetReq {
    pub risk_level: u8,
}
...
pub async fn admin_risk_level_set(
    ...
    let ix = match build_instruction_set_risk_level(&SetRiskLevelParams {
        program_id,
        governance: payer.pubkey(),
    ...
    let _ = db::insert_audit_log(
        ...
        None,
        "risk_level_set",
        serde_json::json!({ "risk_level": req.risk_level, "signature": signature }),
    )
    .await;
...
pub async fn twofa_setup(
    ...
    if let Err(e) = db::twofa_upsert(&state.pool, &req.owner, &secret, false).await {
        return (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(serde_json::json!({ "error": e.to_string() })),
        );
...
pub async fn twofa_verify(
    ...
    let row = match db::twofa_get(&state.pool, &req.owner).await {
        Ok(s) => s,
        Err(e) => {
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn vault_limits(
    ...
    (
        StatusCode::OK,
        Json(serde_json::json!({ "window_seconds": 86400, "used": used, "limit": null })),
    )
}
...
pub async fn analytics_distribution(State(state): State<AppState>) -> impl IntoResponse {
    let rows = sqlx::query_as::<_, VaultTotalRow>("SELECT total_balance FROM vaults")
        .fetch_all(&state.pool)
        .await;
    match rows {
        Ok(rs) => {
...
pub async fn analytics_utilization(State(state): State<AppState>) -> impl IntoResponse {
    let rows = sqlx::query_as::<_, VaultUtilizationRow>(
        "SELECT total_balance, locked_balance FROM vaults",
    )
    .fetch_all(&state.pool)
    .await;
    match rows {
        Ok(rs) => {
            ...
            let utilization = if total > 0 {
                (locked as f64) / (total as f64)
            } else {
                0.0
            };
            (
                StatusCode::OK,
                Json(serde_json::json!({ "utilization": utilization })),
            )
        }
        Err(e) => (
            StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn vault_request_withdraw(
    State(state): State<AppState>,
    Json(req): Json<RequestWithdrawReq>,
) -> impl IntoResponse {
    let message = format!(
        "request_withdraw:{}:{}:{}",
        req.owner, req.amount, req.nonce
    );
    if let Err(e) = verify_wallet_signature(&req.owner, message.as_bytes(), &req.signature) {
        return (
            StatusCode::UNAUTHORIZED,
    ...
    let owner = Pubkey::from_str(&req.owner).unwrap();
    let ix = match build_instruction_request_withdraw(&program_id, &owner, req.amount) {
        Ok(ix) => ix,
        Err(e) => {
            return (
                StatusCode::BAD_REQUEST,
